...
## Edge Function Demo — Quick Summary

This project is a minimal demo. The home page (`/`) fetches `/api/greeting` (a Netlify Edge Function) and displays the visitor's geolocation and the serving edge region. The edge function lives in `netlify/edge-functions/greeting.ts` and uses the Deno runtime.

**Key paths:**
- `netlify/edge-functions/greeting.ts` — edge function; returns geo + region JSON at `/api/greeting`
- `src/routes/index.tsx` — home page; calls the edge function via `fetch` and renders the result

**Non-obvious:** Geolocation data (`context.geo`) is only populated on Netlify's platform. It returns empty/undefined values during local `netlify dev` sessions unless manually spoofed.
